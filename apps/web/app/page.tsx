...
export default function Dashboard() {
  ...
  const { telemetry, isAutopilot, setTelemetry, setAutopilot } =
    useRobotStore();
        ...
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-slate-400 mt-1">
              Single robot monitoring & control
            </p>
          </div>
          ...
          <div className="flex items-center gap-4">
            ...
            {telemetry && (
              <Badge
                variant={
                  telemetry.status === "autopilot" ? "default" : "secondary"
                }
              >
            ...
            <Card className="p-4">
              ...
              {telemetry && (
                <div className="mt-4 text-sm text-slate-400 font-mono">
                  <p>
                    Position: x={telemetry.pose.x.toFixed(2)}m, y=
                    {telemetry.pose.y.toFixed(2)}m, θ=
                    {telemetry.pose.theta.toFixed(2)}rad
                  </p>
                  <p>
                    Speed: L={telemetry.speed.leftRps.toFixed(2)} R=
                    {telemetry.speed.rightRps.toFixed(2)} rps
                  </p>
                  {telemetry.battery && (
                    <p className="flex items-center gap-2">
                      <Zap className="w-4 h-4" />
                      Battery: {telemetry.battery.voltage.toFixed(1)}V (
